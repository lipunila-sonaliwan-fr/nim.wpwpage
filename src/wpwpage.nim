...
proc attachToDesktop(hwnd: HWND, workerW: HWND, screenW, screenH: int32) =
  ...
  style = style and not (WS_CAPTION or WS_THICKFRAME or WS_SYSMENU or
                          WS_MAXIMIZEBOX or WS_MINIMIZEBOX) or WS_POPUP or WS_VISIBLE
  ...
  discard showWindow(hwnd, SW_MAXIMIZE)

# Monitors (in the background) and reattaches if necessary.
proc watchdog(hwnd: HWND, screenW, screenH: int32) =
  while true:
    sleep(4000)
...
proc main() =
  ...
  let hwnd = cast[HWND](webview_get_window(wv.handle))
  attachToDesktop(hwnd, workerW, screenW, screenH)
  ...
  wv.run()
  # m.awaitAll:
  # m.spawn watchdog(hwnd, screenW, screenH)
